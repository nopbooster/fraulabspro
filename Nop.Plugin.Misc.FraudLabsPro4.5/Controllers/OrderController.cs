...
namespace Nop.Plugin.Misc.FraudLabsPro.Controllers
{
    public class OrderController : BaseAdminController
    {
        ...
        public async Task<IActionResult> FraudLabsProOrderScreen(int orderId)
        {
            ...
            if (order != null)
            {
                var orderResult = await _fraudLabsProManager.ScreenOrderAsync(order);
        ...
        public async Task<IActionResult> FraudLabsProOrderApprove(string transactionId, int orderId)
        {
            ...
            {
                var orderResult = await _fraudLabsProManager.OrderFeedbackAsync(orderId, transactionId, Order.Action.APPROVE);
        ...
        public async Task<IActionResult> FraudLabsProOrderReject(string transactionId, int orderId)
        {
            ...
            if (!string.IsNullOrEmpty(transactionId))
            {
                var orderResult = await _fraudLabsProManager.OrderFeedbackAsync(orderId, transactionId, Order.Action.REJECT);
        ...
        public async Task<IActionResult> FraudLabsProOrderBlackList(string transactionId, int orderId)
        {
            ...
            if (!string.IsNullOrEmpty(transactionId))
            {
                var orderResult = await _fraudLabsProManager.OrderFeedbackAsync(orderId, transactionId, Order.Action.REJECT_BLACKLIST);
