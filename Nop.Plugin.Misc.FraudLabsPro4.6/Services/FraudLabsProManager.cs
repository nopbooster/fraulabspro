...
namespace Nop.Plugin.Misc.FraudLabsPro.Services
{
    public class FraudLabsProManager
    {
        ...
        private readonly IRepository<NopOrder> _orderRepository;
        ...
        public FraudLabsProManager(
            ...
            IRepository<NopOrder> orderRepository,
        ...
        public async Task UpdateOrerStatusAsync(NopOrder order, string fraudLabsProStatus)
        {
        ...
        public async Task HandleOrderPlacedEventAsync(NopOrder order)
        {
            if (order != null)
               await ScreenOrderAsync(order);
        }
        ...
        public async Task<OrderResult> ScreenOrderAsync(NopOrder order)
        {
            ...
            try
            {
                ...
                if (customer != null)
                {
                    ...
                    // Order Information
                    screenOrderPara.Department = _storeContext.GetCurrentStore().Name ?? string.Empty;
                    ...
                    await UpdateOrerStatusAsync(order, result.FraudLabsProStatus);
        ...
        public async Task<OrderResult> OrderFeedbackAsync(int orderId, string transactionId, string actionName)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(result.FraudLabsProErrorCode) && string.IsNullOrEmpty(result.FraudLabsProMessage))
                {
                    ...
                    await UpdateOrerStatusAsync(order, actionName);
        ...
        /// <summary>
        /// Delete order result generic attributes
        /// </summary>
        /// <returns></returns>
        public async Task DeleteOrderResultGenericAttributes()
        {
            var i = 0;
            while (true)
            {
                var query = from ga in _genericAttributeRepository.Table
                            join o in _orderRepository.Table on ga.EntityId equals o.Id
                            where ga.KeyGroup == "Order" &&
                            (ga.Key.Equals("FraudLabsProOrderResult") || ga.Key.Equals("FraudLabsProOrderStatus"))
                            select ga;

                //don't load all available generic attributes at a time
                var list = await query.ToPagedListAsync(i, 1000);
                if (!list.Any())
                    break;

                await _genericAttributeRepository.DeleteAsync(list);
                i++;
            }
        }
